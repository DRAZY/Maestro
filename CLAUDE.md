...
- **Audio/video playback:** the `<audio>`/`<video>` element is owned by `MediaPlaybackHost` (`src/renderer/components/MediaPlayback/`), mounted once in `App.tsx`, NOT by `FilePreview`. `FilePreview` renders a `MediaViewportSlot` that publishes its rect via `mediaPlaybackStore`. Do NOT move the element into the tab: `MainPanelContent` unmounts `FilePreview` on every tab/agent switch, and removing a media element from the document pauses it. Classify media with `getFileTabMediaKind()` in `src/renderer/utils/mediaTabs.ts`.
