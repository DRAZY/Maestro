...
- **Run a shell command from the chat:** `runShellCommand()` / `cancelShellCommand()` in `src/renderer/services/shellCommand.ts`, with `parseShellCommandInput()` / `stripShellCommandEscape()` in `src/renderer/utils/shellCommandInput.ts`. This is command mode (`!command` in the AI composer). Any new surface that runs a one-off command and shows its output in a transcript should reuse this rather than calling `window.maestro.process.runCommand` directly - the synthetic `{sessionId}-shell-{runId}` id is what keeps the output out of the agent listeners. See [RENDERER-SERVICES.md](docs/agent-guides/RENDERER-SERVICES.md#shellcommandts-200-lines).
