...
- ❗ **[Command Mode](./general-usage#command-mode)** - Start any chat message with `!` to run it as a shell command instead of sending it to the agent. `!git pull`, `!ls`, `!npm test` run in the agent's working directory (or on its SSH remote) and stream their output into the transcript, even while the agent is mid-turn.
